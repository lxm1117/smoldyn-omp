	...
	for(prd=0;prd<nprod;prd++) {
		...
		if(rxn->rparamt==RPconfspread) {
			...
			if(prd==0) {
				...
				dist=sqrt(dist); }			// dist is separation between centers
			if(rxn->rparam>=0)
				for(d=0;d<dim;d++)
					mptr->pos[d]=rxnpos[d]-(mptr2->pos[d]-mptr1->pos[d])/dist*rxn->prdpos[prd][0];
			else
				for(d=0;d<dim;d++)
					mptr->pos[d]=rxnpos[d]-(2*sqrt(rxn->bindrad2)-dist)*(mptr2->pos[d]-mptr1->pos[d])/dist*rxn->prdpos[prd][0];
			mptr->pnl=(mptr->mstate==MSsoln)?NULL:mptrallo->pnl; }

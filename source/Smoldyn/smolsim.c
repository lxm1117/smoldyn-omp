...
int simulatetimestep(simptr sim) {
	...
	#pragma omp barrier
	tmp_time1=omp_get_wtime();
	//printf("thread_id=%d, time0=%f, time1=%f, dur=%f\n", omp_get_thread_num(), tmp_time0, tmp_time1, tmp_time1-tmp_time0);
	...
	//#pragma omp master  
		...
		*/
		...
		#pragma omp master
		{
			...
			//printf("time update thread=%d, sim->time=%f, tmax=%f\n", omp_get_thread_num(),sim->time,sim->tmax);													// --- end of time step ---
		}
		#pragma omp barrier
...
int smolsimulate(simptr sim) {
	...
	if(!er)
		...
		do{
			#pragma omp parallel			
			{
		   	 	//printf("probe 0, th_id= %d, sim=%p\n", omp_get_thread_num(), (void*)sim);

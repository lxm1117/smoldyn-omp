...
int molsetdifm(simptr sim,int ident,int *index,enum MolecState ms,double *difm) {
	...
	for(ms=mslo;ms<mshi;ms=(enum MolecState)(ms+1)) {
		...
		for(d=0;d<sim->dim*sim->dim;d++)
			difmat[d]=difm[d];
		dotMMD(difmat,difmat,dm2,dim,dim,dim);  //in lib/Rn.h
...
void molssfree(molssptr mols,int maxsrf) {
	...
	free(mols->expand);		mols->expand=NULL;	

	free(mols->gausstbl);	mols->gausstbl=NULL;
...
int diffuse(simptr sim) {
	...
	double **difstep,***difm,***drift, *gtable;
	double epsilon,margin,neighdist,dt;
	//moleculeptr *mlist;
		...
		if(sim->mols->diffuselist[ll]) {
			...
			for(m=2;m<nmol;m++) {
				...
				*/
				...
				//if(!mols->difm[i][ms]){																		// isotropic diffusion
				//	for(d=0;d<dim;d++)
				//		mptr->pos[d]+=mols->difstep[i][ms]*mols->gausstbl[randULI()&ngtablem1];
				...
				/*
					...
					for(d=0;d<3;d++){
						...
						//v2[d]=0;
						for(j=0;j<dim;j++) { v2[d]=difm[i][ms][dim*d+j]*v1[j];}	
						mptr->pos[d]+=v2[d];	
					}
				    //dotMVD(difm[i][ms],v1,v2,dim,dim);  //in /lib/Rn.c
					for(d=0;d<dim;d++) mptr->pos[d]+=v2[d]; 
				}
				if(drift[i][ms])
					//for(d=0;d<dim;d++) mptr->pos[d]+=drift[i][ms][d]*dt;
					for(d=0;d<3;d++) mptr->pos[d]+=drift[i][ms][d]*dt;
				if(mptr->mstate!=MSsoln)														// neighbor panel moving
